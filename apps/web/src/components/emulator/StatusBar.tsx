...
export function StatusBar({ time, battery = 85, wifi = true, signal = true }: StatusBarProps) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-1 text-xs text-white bg-black/20">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <div className="w-5 h-2.5 border border-white/60 rounded-sm relative">
            <div
              className={`absolute left-0.5 top-0.5 bottom-0.5 rounded-[1px] ${
                battery > 20 ? 'bg-green-400' : 'bg-red-400'
              }`}
              style={{ width: `${Math.max(0, battery - 10)}%` }}
            />
          </div>
        </div>
      </div>
