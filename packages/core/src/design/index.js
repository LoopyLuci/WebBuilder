...
export class DesignEngine {
     ...
     */
    generateComponentCSS(componentName, variant) {
    ...
    getTokenValue(path) {
        ...
        let current = this.system.tokens;
     ...
     */
    setTokenValue(path, value, type = 'string') {
        const parts = path.split('.');
        let current = this.system.tokens;
...
export default {
  ...
  plugins: [],
    ...
    generateTokenVariables(tokens, indent, lines) {
    ...
    flattenFonts(fonts) {
        ...
        for (const [name, token] of Object.entries(fonts)) {
            if ('value' in token) {
                result[name] = token.value.split(',').map(s => `'${s.trim()}'`);
            }
        }
        return result;
    }
...
// ─── Utility Functions ──────────────────────────────────────────────────────
export function createDesignEngine(system) {
    return new DesignEngine(system);
}
export function createDesignTokens(overrides) {
