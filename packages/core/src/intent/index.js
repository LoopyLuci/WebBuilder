...
export class IntentParser {
    ...
    generateProjectName(intent) {
        ...
        if (purpose.includes('shop') || purpose.includes('sell'))
        ...
        if (purpose.includes('tool') || purpose.includes('service'))
            return 'Web Application';
        if (purpose.includes('inform'))
            return 'Landing Page';
        return 'My Project';
    }
    /**
     ...
     */
    generateDeploymentConfig(intent) {
     ...
     */
    identifyClarifications(intent, spec) {
        const questions = [];
        // Ask about pages
     ...
     */
    calculateConfidence(intent, spec, clarifications) {
        ...
        if (intent.references.length > 0)
            score += 0.1;
        if (intent.audience !== 'General users')
            score += 0.05;
        if (spec.structure && spec.structure.pages.length > 0)
            score += 0.1;
...
export function validateSpec(spec) {
    ...
    if (!spec.intent)
        errors.push('Project intent is required');
    if (!spec.structure || spec.structure.pages.length === 0)
        errors.push('At least one page is required');
    if (!spec.design)
        errors.push('Design system is required');
    if (!spec.functionality)
        errors.push('Feature set is required');
    if (!spec.deployment)
        errors.push('Deployment config is required');
    return {
