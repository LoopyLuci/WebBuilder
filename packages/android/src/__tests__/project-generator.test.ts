...
import type { AndroidProjectConfig } from '../../dist/index.js';

const sampleConfig: AndroidProjectConfig = {
  name: 'Test App',
...
describe('AndroidProjectGenerator', () => {
  ...
  it('should generate Kotlin source files for activities', () => {
    ...
    const mainActivity = result.files.find((f) =>
      f.path.includes('MainActivity.kt')
    );
    ...
    const settingsActivity = result.files.find((f) =>
      f.path.includes('SettingsActivity.kt')
    );
    expect(settingsActivity).toBeDefined();
    expect(settingsActivity!.content).toContain(
